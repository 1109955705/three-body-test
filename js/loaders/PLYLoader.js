...
THREE.PLYLoader.prototype = {
      ...
      switch (lineType) {
        ...
        case "property":
          currentElement.properties.push(
            make_ply_element_property(lineValues, this.propertyNameMapping)
          );
  ...
  parseASCIIElement: function (properties, line) {
    ...
    for (var i = 0; i < properties.length; i++) {
      if (properties[i].type === "list") {
        ...
        for (j = 0; j < n; j++) {
          list.push(
            this.parseASCIINumber(values.shift(), properties[i].itemType)
          );
        }

        element[properties[i].name] = list;
      } else {
        element[properties[i].name] = this.parseASCIINumber(
          values.shift(),
          properties[i].type
        );
      }
  ...
  parseASCII: function (data) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      var element = this.parseASCIIElement(
